...
class TeamBotConfig:
    ...
    # ПЕРЕИМЕНОВАНА из TEAM_METRICS_HOUR осознанно. У дайджеста изменилась
    # семантика: раньше это был «вечерний отчёт за текущий день», теперь —
    # ночной отчёт за уже ЗАКОНЧИВШИЕСЯ сутки. Старое значение (21) осталось в
    # рабочих .env с прежних времён и молча перебивало новый дефолт: после
    # git pull и рестарта сводка всё равно приходила в 21:00, потому что
    # переменная окружения сильнее кода. Новое имя означает, что стухшее
    # значение просто перестаёт применяться, и обновления кода достаточно —
    # лезть в .env на сервере не нужно. Наличие старой переменной не
    # игнорируется молча: см. legacy_metrics_hour_env ниже.
    metrics_hour: int = field(default_factory=lambda: int(_optional("TEAM_METRICS_HOUR_MSK", "0")))

    # Только для предупреждения в логе: если старая переменная где-то ещё
    # осталась, человек должен узнать, что она больше ни на что не влияет.
    legacy_metrics_hour_env: str = field(default_factory=lambda: _optional("TEAM_METRICS_HOUR"))
