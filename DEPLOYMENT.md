...
### Шаг 0a — сгенерировать свой SSH-ключ (у себя, ещё ДО обращения к Кириллу)

Если на своей машине ключа ещё нет (проверить: `ls ~/.ssh/*.pub`) —
сгенерировать:
```bash
ssh-keygen -t ed25519 -C "sasha-kubyshka-deploy"
cat ~/.ssh/id_ed25519.pub
```
Этот публичный ключ — то, что отправляется Кириллу (шаг 0b), а не пароль.
Ключ вместо пароля даёт вход БЕЗ ввода пароля на каждый `ssh`/`git push` —
особенно важно для варианта B (деплой через `git push`, см. ниже), где
пароль пришлось бы вводить на каждое обновление.

### Шаг 0b — что запросить у Кирилла

Можно переслать ему как есть:

> Кирилл, чтобы закинуть ботов на сервер, нужно от тебя:
> 1. **Доступ**: IP-адрес или домен сервера + логин пользователя, под
>    которым заходить. Плюс один из двух: (а) добавь мой SSH-ключ в
>    `~/.ssh/authorized_keys` на сервере — вот он: `<вставить вывод cat
>    ~/.ssh/id_ed25519.pub>` — тогда буду заходить без пароля; либо (б)
>    просто дай пароль от этого пользователя.
> 2. **Права**: есть ли у этого пользователя `sudo`? (нужно для установки
>    пакетов и настройки автозапуска ботов)
> 3. **Что уже крутится на сервере** — в частности, не задеплоен ли там уже
>    Finik-backend (бэкенд самого приложения)? Если да — на каких портах,
>    чтобы не пересечься. И вообще — используется ли сервер для чего-то
>    ещё, о чём стоит знать.
> 4. **ОС** — `cat /etc/os-release` на сервере, пришли вывод (инструкция
>    рассчитана на Ubuntu/Debian, если что-то другое — придётся адаптировать).
> 5. **Домен** (не обязательно, но желательно) — есть ли у тебя
>    домен/поддомен, который можно направить на этот сервер? Нужен для
>    HTTPS-адреса доски задач (Telegram не открывает такие кнопки без
>    HTTPS). Если домена нет — сделаем через Cloudflare-туннель, тогда
>    дополнительно нужен доступ к твоему Cloudflare-аккаунту (или заведём
>    отдельный).

Секреты самих ботов (Telegram-токены через BotFather, OpenAI API-ключ) —
это НЕ у Кирилла, их получаю/уже имею сам, в списке выше не нужны.
