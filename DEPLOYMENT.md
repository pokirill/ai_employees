...
## Обновление в будущем — КАК ЭТО РАБОТАЕТ СЕЙЧАС (проверено 09.08.2026)

> ⚠️ **Ниже в этом файле описан systemd, но на проде его НЕТ.** Юнитов
> `team-bot`/`channel-bot`/`kubyshka-webapp` не существует (`systemctl list-units`
> по ним пуст), боты запущены через **`screen`**. Поэтому `sudo systemctl restart
> team-bot` выполняется без ошибки и НИЧЕГО не делает — из-за этого правка кода
> однажды «не доехала»: код лежал новый, а в памяти крутился процесс с 2 августа.
> Реальные факты прода: сервер `kubyshka-prod` (`dev2@207.154.252.247`), код в
> `~/apps/ai_employees` (НЕ `kubyshka-bots` — та папка это `.OLD-replaced-*`),
> ветка `claude-work-kubyshka-bots-migration`.

Рабочая последовательность:

```bash
ssh dev2@207.154.252.247
cd ~/apps/ai_employees
git pull --ff-only origin claude-work-kubyshka-bots-migration
source .venv/bin/activate
pip install -r requirements.txt          # если requirements.txt менялся
python -m pytest tests/ -q               # прогнать ПЕРЕД рестартом

# рестарт того бота, чей код менялся (пример для team-bot):
screen -S team-bot -X quit
screen -dmS team-bot bash -c "source .venv/bin/activate && python -m team_bot.main >> team_bot.log 2>&1"

# проверка, что поднялся и подхватил новое:
screen -ls                               # сессия team-bot должна быть Detached
tail -20 team_bot.log                    # 'Bot username resolved' + расписание дайджеста
```

**Проверять надо именно лог, а не факт рестарта.** Бот при старте печатает
эффективное расписание дайджеста (`Дайджест метрик: 00:00 МСК, за уже
закончившиеся сутки`) и предупреждение, если в `.env` осталась устаревшая
переменная. Если сводка приходит «не в то время» — ответ будет в этих строках.

**Помните про `.env` на сервере.** Он не в git и переменные окружения сильнее
дефолтов в коде: смена дефолта в репозитории НЕ меняет поведение прода, если
переменная там задана явно. Именно так `TEAM_METRICS_HOUR=21` держал сводку в
21:00 после обновления кода (переменная с тех пор переименована в
`TEAM_METRICS_HOUR_MSK`, старая игнорируется, но урок общий).

### Незакрытый риск: боты не переживут перезагрузку сервера

`screen`-сессии умирают вместе с машиной, а systemd-юнитов нет — значит после
ребута прода все три процесса просто не поднимутся, и узнаем мы об этом по
тишине в чате. Ниже в этом файле есть готовые юниты (шаг 7) — их стоит
завести, но это отдельная задача: неаккуратный перенос уронит работающих ботов.

## Обновление через systemd (ПЛАН, на проде пока не применён)
